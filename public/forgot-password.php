...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($email === '') {
    ...
    $pdo = db();
    $stmt = $pdo->prepare('SELECT email FROM couples WHERE email = ?');
    $stmt->execute([$email]);
    if ($stmt->fetch()) {
      $token = bin2hex(random_bytes(32));
      $expires = date('Y-m-d H:i:s', time() + 3600); // 1 hour

      $pdo->prepare('UPDATE couples SET reset_token = ?, reset_expires = ? WHERE email = ?')
...
<title>Forgot Password - Forever Together</title>
<style>
...
.local-link-box {padding:15px; background: #fff; border: 1px dashed var(--gold); border-radius: 4px; margin-bottom: 20px; font-size: 13px; word-break: break-all;}
...
<div class="left">
  ...
  <div class="left-mid">
    ...
    <p class="left-desc">Reset your password to get back to planning and preserving your timeless memories.</p>
  </div>
</div>
...
<div class="right">
  <div class="form-card">
    ...
    <p class="form-sub">Enter your email and we'll generate a password reset link for you.</p>
    ...
    <?php if ($success): ?>
      ...
      <?php if ($resetLink): ?>
        <div class="local-link-box">
          <strong>Local Dev Note:</strong> Email sending is bypassed. Click the link to reset your password:<br><br>
          <a href="<?= htmlspecialchars($resetLink) ?>" style="color:var(--gold); font-weight: 500; text-decoration: underline;"><?= htmlspecialchars($resetLink) ?></a>
        </div>
      <?php endif; ?>
    <?php endif; ?>
    ...
    <form method="post">
      ...
      <button type="submit" class="btn-submit">Generate Reset Link &rarr;</button>
    </form>
    
    <p class="switch-link">Remembered your password? <a href="couple-login.php">Sign In</a></p>
  </div>
</div>
</body>
</html>